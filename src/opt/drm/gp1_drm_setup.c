...
static int drm_select_best_mode(struct gp1_video *driver,drmModeResPtr res) {
  //fprintf(stderr,"%s...\n",__func__);
  ...
  for (;conni<res->count_connectors;conni++) {
    ...
    for (;modei<conn->count_modes;modei++,mode++) {
      ...
      if (drm_pick_mode(driver,bestconn,&bestmode,conn,mode)>0) {
...
int gp1_drm_configure(struct gp1_video *driver) {
  ...
  if (drm_select_best_mode(driver,res)<0) {
